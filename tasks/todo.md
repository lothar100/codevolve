...
| REVIEW-08-IMPL08 | Iris | [!] | Review IMPL-08 + IMPL-09 — verify analytics separation, no primary DB writes, query correctness, idempotency logic, DLQ configuration, schema correctness against all 5 DESIGN-02 dashboard queries. **CHANGES REQUIRED 2026-03-23 (Iris):** CRITICAL: `clickhouseClient.ts` line 55 constructs `url: \`https://${secret.host}:...\`` but secret's `host` field already contains `https://` — double-protocol URL will break all ClickHouse connections. Fix before deploy. Warnings: (W-01) AnalyticsConsumerFn timeout 300s should be 60s per spec. **W-01 fixed 2026-03-23:** AnalyticsConsumerFn timeout reduced 300s→60s. (W-02) DLQ alarm uses wrong metric (`metricNumberOfMessagesSent` vs `metricApproximateNumberOfMessagesVisible`). **W-02 fixed 2026-03-23:** DLQ alarm metric changed from `metricNumberOfMessagesSent()` to `metricApproximateNumberOfMessagesVisible({ period: 1min })`. (W-03) eventId.ts uses `"null"` sentinel for null fields — spec says `""`, update spec to match code. **W-03 fixed 2026-03-23** (see IMPL-08 notes). (W-04) dashboard SQL `from`/`to` params not sanitized before interpolation. **W-04 fixed 2026-03-23** (see IMPL-09 notes). All 34 analytics tests pass. See docs/review-08-impl08.md. | IMPL-08, IMPL-09 |
...
| IMPL-15 | Ada | [~] | Implement MCP server per DESIGN-06: tools for resolve/execute/chain/validate/list/get, resources for skills/problems. Depends on DESIGN-06. stdio server, mcp-config.json, 28 unit tests. **Scaffold complete 2026-03-22.** | DESIGN-06 |
| IMPL-16 | Ada | [~] | Implement community auth (Cognito) + per-user trusted mountain (saved skill sets). CommunityUserPool + UserPoolClient CDK, CognitoUserPoolsAuthorizer on write endpoints, backup JWT Lambda. **Scaffold complete 2026-03-22.** | — |
