...
| IMPL-10 | Ada | [!] | Implement Decision Engine Lambda (scheduled): auto-cache trigger, optimization flag, gap detection → SQS GapQueue, archive evaluation → SQS ArchiveQueue. **REVIEW-08 W-01 RESOLVED (2026-03-25):** `optimizationFlag.ts` now uses `QueryCommand` on `GSI-status-updated` — ScanCommand regression fixed. **REVIEW-08 W-02 RESOLVED (2026-03-25):** `ARCHIVE_QUEUE_URL` fallback is now `""` (placeholder account ID removed). **CRITICAL (REVIEW-15, 2026-03-25):** CDK scaffold for Decision Engine is absent from `infra/codevolve-stack.ts`. `DecisionEngineFn`, `DecisionEngineSchedule` (EventBridge), `GapLogTable`, all IAM grants, and all env var injections are missing — Lambda cannot be scheduled or deployed. This is a regression from the state REVIEW-08 approved. Must restore CDK constructs and verify `npx cdk synth` confirms `DecisionEngineFn`, `DecisionEngineSchedule`, `GapLogTable` in template with `reservedConcurrentExecutions: 1`. Lambda source code is complete and correct. 54 tests pass, tsc clean. W-03 (staleness thresholds not runtime-configurable) remains deferred to Phase 3. See `docs/reviews/REVIEW-15-IMPL10.md`. | ARCH-07, DESIGN-03 |
