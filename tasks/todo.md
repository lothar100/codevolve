...
| IMPL-15 | Ada | [!] | Implement MCP server per DESIGN-06: tools for resolve/execute/chain/validate/list/get, resources for skills/problems. Depends on DESIGN-06. stdio server, mcp-config.json, 28 unit tests. **Scaffold complete 2026-03-22.** **REJECTED 2026-03-25 (REVIEW-12):** CRITICAL-01: `@modelcontextprotocol/sdk` missing from package.json — server cannot start. CRITICAL-02: 4 of 6 tool names wrong (must be resolve_skill/execute_skill/chain_skills/validate_skill). CRITICAL-03: `submit_skill` tool absent (6th tool per DESIGN-06). CRITICAL-04: `server.ts` does not export `createServer` — server.test.ts fails to run. CRITICAL-05: `tsc --noEmit` fails (8 implicit any + 2 missing modules). See `docs/reviews/REVIEW-12-IMPL15.md`. | DESIGN-06 |
