...
| IMPL-08 | Ada | [~] | Implement analytics event consumer: Kinesis → Lambda → ClickHouse/BigQuery. Batch writes, dead-letter queue, idempotent processing. **Implemented 2026-03-25 (Ada):** `src/analytics/consumer.ts` (Kinesis handler, two-phase parse+insert), `src/analytics/clickhouseClient.ts` (env-var singleton), `src/analytics/eventId.ts` (SHA-256 dedup ID), `src/analytics/toClickHouseRow.ts` (event → ClickHouseRow). CDK resources in `infra/codevolve-stack.ts` (AnalyticsConsumerFn, AnalyticsConsumerDlq, AnalyticsConsumerDlqAlarm, KinesisEventSource). **Fixed 2026-03-25 (Ada):** CDK now injects CLICKHOUSE_URL/USER/PASSWORD/DATABASE from Secrets Manager; confidence sentinel fixed to -1.0 (W-01); pre-insert dedup SELECT added per spec §5.3 (W-02). Pending Iris re-review. | ARCH-07, DESIGN-02 |
| IMPL-09 | Ada | [~] | Implement 5 dashboard API endpoints (read from ClickHouse/BigQuery). **Implemented 2026-03-25 (Ada):** `src/analytics/dashboards.ts` with all 5 dashboard types (resolve-performance, execution-caching, skill-quality, evolution-gap, agent-behavior). ISO8601 validation on from/to params. 14 unit tests pass. Pending Iris review. | IMPL-08, DESIGN-02 |
...
| REVIEW-08-IMPL08 | Iris | [!] | Review IMPL-08 + IMPL-09 — verify analytics separation, no primary DB writes, query correctness, idempotency logic, DLQ configuration, schema correctness. **CHANGES REQUIRED (Iris):** NEW CRITICAL: CDK did not inject CLICKHOUSE_URL/USER/PASSWORD/DATABASE into analyticsConsumerFn — CLICKHOUSE_SECRET_ARN was dead. New W-01: confidence null sentinel mismatch (spec DDL Float64 -1.0 vs code TypeScript null). New W-02: pre-insert dedup check absent (spec §5.3). 48 analytics tests pass. See docs/reviews/REVIEW-08-IMPL08-RECHECK.md. **Fixed 2026-03-25 (Ada):** CDK now injects CLICKHOUSE_URL/USER/PASSWORD/DATABASE from Secrets Manager secret fields (CLICKHOUSE_SECRET_ARN env var removed); confidence sentinel fixed to -1.0 (W-01 — Float64 non-nullable column); pre-insert dedup SELECT added before INSERT per spec §5.3 (W-02). tsc --noEmit passes, 284 tests pass. Pending Iris re-review. | IMPL-08, IMPL-09 |
