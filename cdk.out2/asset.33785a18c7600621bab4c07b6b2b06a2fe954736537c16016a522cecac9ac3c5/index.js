...
var PROBLEMS_TABLE = process.env.PROBLEMS_TABLE ?? "codevolve-problems";
var SKILLS_TABLE = process.env.SKILLS_TABLE ?? "codevolve-skills";
var CACHE_TABLE = process.env.CACHE_TABLE ?? "codevolve-cache";
var ARCHIVE_TABLE = process.env.ARCHIVE_TABLE ?? "codevolve-archive";
var EVOLVE_JOBS_TABLE = process.env.EVOLVE_JOBS_TABLE ?? "codevolve-evolve-jobs";
...
var DashboardTypeSchema = external_exports.enum([
  "resolve-performance",
  "execution-caching",
  "skill-quality",
  "evolution-gap",
...
var SupportedLanguageSchema = external_exports.enum([
  "python",
  "javascript",
  "typescript",
  "go",
...
var SkillInputSchema = external_exports.object({
  name: external_exports.string().min(1).max(128),
  type: external_exports.string().min(1).max(128)
});
var SkillOutputSchema = external_exports.object({
  name: external_exports.string().min(1).max(128),
  type: external_exports.string().min(1).max(128)
});
var SkillExampleSchema = external_exports.object({
  input: external_exports.record(external_exports.unknown()),
  output: external_exports.record(external_exports.unknown())
});
var SkillTestSchema = external_exports.object({
  input: external_exports.record(external_exports.unknown()),
  expected: external_exports.record(external_exports.unknown())
});
var SkillSchema = external_exports.object({
  skill_id: external_exports.string().uuid(),
  problem_id: external_exports.string().uuid(),
  name: external_exports.string().min(1).max(256),
  description: external_exports.string().max(4096),
  version: external_exports.number().int().positive(),
  version_label: external_exports.string().regex(/^\d+\.\d+\.\d+$/).optional(),
  is_canonical: external_exports.boolean(),
  status: SkillStatusSchema,
  language: SupportedLanguageSchema,
  domain: external_exports.array(external_exports.string().min(1).max(64)).min(1).max(16),
  tags: external_exports.array(external_exports.string().min(1).max(64)).max(32),
  inputs: external_exports.array(SkillInputSchema).min(1),
  outputs: external_exports.array(SkillOutputSchema).min(1),
  examples: external_exports.array(SkillExampleSchema).max(32),
  tests: external_exports.array(SkillTestSchema).max(128),
  implementation: external_exports.string().max(1e6),
  confidence: external_exports.number().min(0).max(1),
  latency_p50_ms: external_exports.number().nonnegative().nullable(),
  latency_p95_ms: external_exports.number().nonnegative().nullable(),
  created_at: external_exports.string().datetime(),
  updated_at: external_exports.string().datetime()
});
var ProblemSchema = external_exports.object({
  problem_id: external_exports.string().uuid(),
  name: external_exports.string().min(1).max(256),
  description: external_exports.string().max(8192),
  difficulty: external_exports.enum(["easy", "medium", "hard"]),
  domain: external_exports.array(external_exports.string().min(1).max(64)).min(1).max(16),
  tags: external_exports.array(external_exports.string().min(1).max(64)).max(32),
  constraints: external_exports.string().max(4096).optional(),
  canonical_skill_id: external_exports.string().uuid().nullable(),
...
var AnalyticsEventSchema = external_exports.object({
  ...
  skill_id: external_exports.string().uuid().nullable(),
  intent: external_exports.string().max(1024).nullable(),
  ...
  input_hash: external_exports.string().max(128).nullable(),
  success: external_exports.boolean()
});
var CreateSkillRequestSchema = external_exports.object({
  problem_id: external_exports.string().uuid(),
  name: external_exports.string().min(1).max(256),
  description: external_exports.string().max(4096),
  version_label: external_exports.string().regex(/^\d+\.\d+\.\d+$/).default("0.1.0").optional(),
  status: SkillStatusSchema.default("unsolved"),
  language: SupportedLanguageSchema,
  domain: external_exports.array(external_exports.string().min(1).max(64)).min(1).max(16),
  tags: external_exports.array(external_exports.string().min(1).max(64)).max(32).default([]),
  inputs: external_exports.array(SkillInputSchema).min(1),
  outputs: external_exports.array(SkillOutputSchema).min(1),
  examples: external_exports.array(SkillExampleSchema).max(32).default([]),
  tests: external_exports.array(SkillTestSchema).max(128).default([]),
  implementation: external_exports.string().max(1e6).default("")
});
var CreateProblemRequestSchema = external_exports.object({
  name: external_exports.string().min(1).max(256),
...
var PaginationMetaSchema = external_exports.object({
  limit: external_exports.number().int().positive(),
  next_token: external_exports.string().nullable()
});
...
// src/shared/emitEvent.ts
var import_client_kinesis = require("@aws-sdk/client-kinesis");
var kinesisClient = new import_client_kinesis.KinesisClient({
  region: process.env.AWS_REGION ?? "us-east-2"
});
var EVENTS_STREAM = process.env.EVENTS_STREAM ?? "codevolve-events";
...
function computeInputHash(inputs) {
  ...
  return (0, import_node_crypto.createHash)("sha256").update(canonical, "utf8").digest("hex");
}

// src/execution/runners.ts
...
function getRunnerFunctionName(language) {
...
async function invokeRunner(functionName, payload) {
  ...
  const payloadStr = payloadBytes ? Buffer.from(payloadBytes).toString("utf8") : "{}";
  return {
...
var rawClient = new import_client_dynamodb2.DynamoDBClient({
  region: process.env.AWS_REGION ?? "us-east-2"
});
var docClient2 = import_lib_dynamodb2.DynamoDBDocumentClient.from(rawClient, {
  marshallOptions: { removeUndefinedValues: true }
});
var CACHE_TABLE_NAME = process.env.CACHE_TABLE_NAME ?? "codevolve-cache";
async function getCachedOutput(skillId, inputHash) {
...
async function writeCachedOutput(params) {
...
async function incrementCacheHit(skillId, inputHash) {
  try {
    await docClient2.send(
...
var ExecuteRequestSchema = external_exports.object({
  ...
  skip_cache: external_exports.boolean().optional().default(false),
  timeout_ms: external_exports.number().int().min(100).max(3e5).optional().default(1e4)
});
var ERROR_TYPE_MAP = {
...
function sanitizeStackTrace(detail) {
  ...
  const filteredFrames = frameLines.filter(
    (frame) => !INTERNAL_FRAME_PATTERNS.some((pattern) => pattern.test(frame))
  );
  const cappedFrames = filteredFrames.slice(0, 5);
  const cleanedFrames = cappedFrames.map(
    (frame) => frame.replace(/\/var\/task\//g, "").replace(/\/var\/runtime\//g, "")
  );
  const cleanedMessage = messageLines.join("\n").replace(/\/var\/task\//g, "").replace(/\/var\/runtime\//g, "").replace(/\/[^\s]+\//g, "");
  return [cleanedMessage, ...cleanedFrames].filter(Boolean).join("\n");
}
function classifyFunctionError(functionError, payloadStr) {
  let parsed = {};
  try {
  ...
  if (errorType === "States.Timeout" || errorMessage && /timed out/i.test(errorMessage) || functionError && /timeout/i.test(functionError)) {
    return "timeout";
  }
  if (errorType === "Runtime.ExitError" || typeof errorMessage === "string" && errorMessage.includes("signal: killed") || errorMessage && /out of memory/i.test(errorMessage)) {
    return "oom";
  }
  return "runtime";
}
async function updateSkillStats(skillId, versionNumber, latencyMs, existingP50, existingP95) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const newP50 = existingP50 !== null ? 0.9 * existingP50 + 0.1 * latencyMs : latencyMs;
  let newP95;
  if (existingP95 === null) {
    newP95 = latencyMs;
  ...
  await docClient.send(
    new import_lib_dynamodb3.UpdateCommand({
      ...
      UpdateExpression: "ADD execution_count :one SET last_executed_at = :now, latency_p50_ms = :p50, latency_p95_ms = :p95",
      ExpressionAttributeValues: {
        ":one": 1,
        ":now": now,
        ":p50": newP50,
...
async function handler(event) {
  ...
  const versionNumber = skill.version_number;
  const skillLanguage = skill.language;
  const skillImplementation = skill.implementation;
  const skillConfidence = skill.confidence ?? 0;
  const autoCache = skill.auto_cache ?? false;
  const existingP50 = skill.latency_p50_ms ?? null;
  const existingP95 = skill.latency_p95_ms ?? null;
  const skillInputs = skill.inputs ?? [];
  ...
  if (!skipCache) {
    ...
    if (cachedOutput !== null) {
      ...
      const executionId2 = v4_default();
      incrementCacheHit(skillId, inputHash).catch(
        (e) => console.warn("execute: incrementCacheHit failed (swallowed):", e)
      );
      emitEvent({
        event_type: "execute",
        skill_id: skillId,
        ...
        (e) => console.warn("execute: emitEvent failed (swallowed):", e)
      );
      return success(200, {
        skill_id: skillId,
  ...
  const runnerPayload = {
    ...
    inputs,
    timeout_ms: Math.min(timeoutMs, 1e4)
  };
  let runnerResult;
  try {
  ...
  if (functionError) {
    ...
    return error(
      ...
      code,
      `Execution ${errorType}`,
      errorDetail ? { error_detail: errorDetail } : void 0
    );
  }
  let runnerBody;
  try {
    runnerBody = JSON.parse(payloadStr);
  } catch {
    emitEvent({
      event_type: "execute",
      skill_id: skillId,
  ...
  if (typeof runnerBody.error === "string" && typeof runnerBody.error_type === "string") {
    const errorType = runnerBody.error_type in ERROR_TYPE_MAP ? runnerBody.error_type : "runtime";
    const { status, code } = ERROR_TYPE_MAP[errorType];
    const errorDetail = sanitizeStackTrace(
      runnerBody.stack ?? runnerBody.error
    );
    emitEvent({
      event_type: "execute",
      skill_id: skillId,
  ...
  if (autoCache) {
    writeCachedOutput({
      ...
      (e) => console.warn("execute: writeCachedOutput failed (swallowed):", e)
    );
  }
