...
var promoteCanonical_exports = {};
__export(promoteCanonical_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(promoteCanonical_exports);
...
function validatePromotionGate(skill) {
  ...
  if (testPassCount === void 0 || testPassCount === null || testPassCount === 0) {
    return {
      valid: false,
      status: 422,
...
// src/registry/promoteCanonical.ts
var PathParamsSchema = external_exports.object({
...
async function handler(event) {
  try {
    ...
    const skillItem = skillResult.Items?.[0];
    if (!skillItem) {
      return error(404, "NOT_FOUND", `Skill ${skillId} not found`);
    }
    const gateInput = {
      is_canonical: skillItem.is_canonical,
      confidence: skillItem.confidence,
      test_fail_count: skillItem.test_fail_count,
      test_pass_count: skillItem.test_pass_count,
      ...
      archived: skillItem.archived
    };
    const gate = validatePromotionGate(gateInput);
    if (!gate.valid) {
      return error(gate.status, gate.code, gate.message);
    }
    const problemId = skillItem.problem_id;
    const language = skillItem.language;
    const versionNumber = skillItem.version_number;
    const now = (/* @__PURE__ */ new Date()).toISOString();
    let demotedSkillId = null;
    let demotedVersionNumber = null;
    const canonicalQueryVerified = await docClient.send(
      new import_lib_dynamodb2.QueryCommand({
        TableName: SKILLS_TABLE,
        IndexName: "GSI-canonical",
        KeyConditionExpression: "is_canonical_status = :ics AND problem_id = :pid",
    ...
    const prevCanonical = allCanonicalItems[0];
    if (prevCanonical) {
      demotedSkillId = prevCanonical.skill_id;
      demotedVersionNumber = prevCanonical.version_number;
    }
    const isCanonicalStatus = `true#optimized`;
    const transactItems = [
      // 5a: Promote the skill
      {
        Update: {
          ...
          UpdateExpression: "SET is_canonical = :true, is_canonical_status = :ics, #status = :optimized, updated_at = :now",
      ...
      // 5c: Update problems table — set canonical_skill_id
      {
        Update: {
          ...
          UpdateExpression: "SET canonical_skill_id = :sid, updated_at = :now",
          ExpressionAttributeValues: {
            ":sid": skillId,
            ":now": now
          },
    ...
    if (demotedSkillId !== null && demotedVersionNumber !== null) {
      transactItems.push({
        Update: {
          ...
          UpdateExpression: "SET is_canonical = :false, updated_at = :now REMOVE is_canonical_status",
          ExpressionAttributeValues: {
            ":false": false,
            ":now": now
          },
    ...
    try {
      await docClient.send(
        new import_lib_dynamodb2.TransactWriteCommand({ TransactItems: transactItems })
      );
    } catch (txErr) {
      const err_ = txErr;
      if (err_.name === "TransactionCanceledException" || (err_.message ?? "").includes("TransactionCanceledException")) {
        const reasons = err_.CancellationReasons;
    ...
    const promotedItem = refetchResult.Items?.[0];
    if (!promotedItem) {
      return error(500, "INTERNAL_ERROR", "Failed to re-fetch promoted skill");
    }
    const promotedSkill = mapSkillFromDynamo(promotedItem);
    const response = {
      skill: promotedSkill,
...
async function invalidateCloudFrontPaths(skillId, problemId, demotedSkillId) {
...
function mapSkillFromDynamo(item) {
  return {
