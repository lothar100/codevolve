...
var BEDROCK_MODEL_ID = "amazon.titan-embed-text-v2:0";
var EMBEDDING_DIMENSIONS = 1024;
var MAX_EMBEDDING_TEXT_CHARS = 8192;
var bedrockClient = new import_client_bedrock_runtime.BedrockRuntimeClient({
  region: process.env.AWS_REGION ?? "us-east-2"
});
function buildEmbeddingText(fields) {
  const domainSuffix = fields.domain.length > 0 ? ` domain:${fields.domain.join(" ")}` : "";
  const tagSuffix = fields.tags.length > 0 ? ` tags:${fields.tags.join(" ")}` : "";
  ...
  const description = fields.description.length > maxDescLen ? fields.description.slice(0, maxDescLen) : fields.description;
...
function extractHttpStatus(err) {
  if (err !== null && typeof err === "object" && "$metadata" in err && typeof err.$metadata.httpStatusCode === "number") {
    return err.$metadata.httpStatusCode;
  }
  return 500;
}
function makeEmbeddingError(code, statusCode, cause) {
  const message = code === "EMBEDDING_THROTTLED" ? "Bedrock embedding service is throttling requests" : "Bedrock embedding service returned an error";
...
var TAG_BOOST_PER_MATCH = 0.05;
var DOMAIN_BOOST_PER_MATCH = 0.1;
var MAX_TOTAL_BOOST = 0.2;
...
var ResolveRequestSchema = external_exports.object({
  intent: external_exports.string().min(1).max(1e3),
  language: external_exports.string().optional(),
  domain: external_exports.array(external_exports.string()).default([]),
...
var PROJECTION = "skill_id, version_number, #nm, description, #lang, #st, is_canonical, confidence, #dom, tags, embedding";
var EXPR_NAMES = {
  "#st": "status",
  "#nm": "name",
  "#lang": "language",
...
async function handler(event) {
  ...
  try {
    ...
    void emitEvent({
      ...
      (emitErr) => console.warn("[resolve] emitEvent failed (swallowed):", emitErr)
    );
  ...
  const topMatches = scored.slice(0, req.top_k).map((s) => ({
  ...
  const resolveConfidence = topMatches.length > 0 ? topMatches[0].confidence : 0;
  const evolveTriggered = topMatches.length === 0 || resolveConfidence < effectiveThreshold;
  const bestMatch = resolveConfidence >= effectiveThreshold ? topMatches[0] ?? null : null;
...
async function fetchCandidates(language) {
  const items = [];
  if (language) {
    let lastKey;
    do {
