 ...
 *
 * Produces a deterministic, structured prompt that asks Claude to return a
 * single JSON object matching the skill creation schema.  The output is
 * always wrapped in a ```json ``` code fence so the caller can extract it
...
/**
 * A condensed view of an existing skill used as a few-shot example inside
 * the prompt.  Only the fields that help Claude understand the expected
 * schema shape are included — full implementation text is omitted to keep
 * the prompt compact.
 */
 ...
 *
 * @param intent       The raw intent string from the SQS message (e.g.
 *                     "sort a list of integers by frequency").
...
- \`tests\` must contain at least 3 test cases, each with an \`input\` object and an
  \`expected\` object whose keys match the output schema.
